 ...
 * 直连节点查询 IAM/TDS 信任，并直接查询 DDO 合约得到 issuer DID 文档与公钥（不经过解析服务）；
 * 撤销状态查询发证方平台地址（VC_REVOCATION_BASE_URL）。
...
function extractJws(raw: string): string {
  const trimmed = raw.trim()
  // 标准 VC 信封 JSON：直接取 proof.jwt。
  try {
    const parsed: unknown = JSON.parse(trimmed)
    const envelope = presentationEnvelopeSchema.safeParse(parsed)
    if (envelope.success) return envelope.data.proof.jwt
  } catch {
    // 不是 JSON，按纯 JWS 文本处理。
  }
